...
$query = 'SELECT `lastName`, `firstName`, DATE_FORMAT(`birthDate`, "%d/%m/%Y") AS `birthDate`,FLOOR( DATEDIFF( NOW(), `birthDate` ) / 365) AS `age`, CASE WHEN `card` = 1 THEN \'Oui\' ELSE \'Non\' END AS `card`, `cardNumber` FROM `clients`';
